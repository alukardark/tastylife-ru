...
?>
	...
	<label><?=GetMessage("MFT_PHONE");?><span class="required_star">*</span>
		<p>
			<input class="mask-phone" name="phone" type="tel" required="required" />
			<span class="fa icon_phone"></span>
		</p>
	</label>
	<input type="hidden" name="PARAMS_HASH" value="<?=$arResult["PARAMS_HASH"]?>">
	<input class="btn btn-default btn-lg" type="submit" value="Заказать звонок" />
	<?$frame->end();?>
</form>
<script>
$("#contact_phone input[type='submit']").click(function(e){
		...
		if((dopusk == false) && (mailTo == false)){
			$.post('/sendMailCallback.php', {action: "send", name: name, phone: phone, event_message_id: 13}, function(data){
