			...
			case 'ADD_TO_COMPARE_LIST':
				...
				if (!isset($_SESSION[$arParams["NAME"]][$arParams["IBLOCK_ID"]]["ITEMS"][$productID]))
				{
					...
					if ($found)
					{
						...
						$kol_t = $_SESSION[$arParams['NAME']][$arParams['IBLOCK_ID']]['ITEMS'][$productID]['kol'];

						// echo $kol_t;

						$id_pict = 	$arElement['PREVIEW_PICTURE'] ?  $arElement['PREVIEW_PICTURE'] : $arElement['DETAIL_PICTURE'];

						$pict = CFile::GetPath($id_pict);
						...
						$_SESSION[$arParams['NAME']][$arParams['IBLOCK_ID']]['ITEMS'][$productID] = array(
							...
							'kol' => 0,
							'pict' => $pict,
							'ed_izm' => $arElement['PROPERTY_ED_IZM_VALUE'],
							'artikul' =>$arElement['PROPERTY_ARTIKUL_VALUE'],
						);
...
if(isset($_GET['id']))
{
	$_SESSION['CATALOG_CART_LIST'][$arParams["IBLOCK_ID"]]['ITEMS'][$_GET['id']]['kol'] = $_SESSION['CATALOG_CART_LIST'][$arParams["IBLOCK_ID"]]['ITEMS'][$_GET['id']]['kol']+$_GET['kol'];
}elseif(isset($_POST['BasketId'])){
	$_SESSION['CATALOG_CART_LIST'][$arParams["IBLOCK_ID"]]['ITEMS'][$_POST['BasketId']]['kol'] = $_POST['kolVal'];
}
// echo "<pre>";
// print_r($_SESSION['CATALOG_CART_LIST'][$arParams["IBLOCK_ID"]]['ITEMS'][$_GET['id']]);
// if(isset($_GET['id']))
// print_r($_SESSION['CATALOG_CART_LIST'][$arParams["IBLOCK_ID"]]["ITEMS"][$_GET['id']]);
// else
// print_r($_SESSION['CATALOG_CART_LIST'][$arParams["IBLOCK_ID"]]["ITEMS"]);
// echo "</pre>";

$kol_all = 0;

foreach($_SESSION['CATALOG_CART_LIST'][$arParams["IBLOCK_ID"]]['ITEMS'] as $aritem_)
{
	$summa_all =  $summa_all + $aritem_['kol']*$aritem_['price'];
	$kol_all = $kol_all + $aritem_['kol'];
}

$_SESSION[$arParams['NAME']][$arParams['IBLOCK_ID']]['kol_all'] = $kol_all;
$_SESSION[$arParams['NAME']][$arParams['IBLOCK_ID']]['summa_all'] = $summa_all;
if($_POST['dataRemove']){

	$calculated = $_SESSION[$arParams['NAME']][$arParams['IBLOCK_ID']]['ITEMS'][$_POST['dataRemove']]['price']*$_SESSION[$arParams['NAME']][$arParams['IBLOCK_ID']]['ITEMS'][$_POST['dataRemove']]['kol'];
	$_SESSION[$arParams['NAME']][$arParams['IBLOCK_ID']]['summa_all'] = $_SESSION[$arParams['NAME']][$arParams['IBLOCK_ID']]['summa_all'] - $calculated;
	$_SESSION[$arParams['NAME']][$arParams['IBLOCK_ID']]['kol_all'] = $kol_all-$_POST['kolVal'];
	unset($_SESSION[$arParams['NAME']][$arParams['IBLOCK_ID']]['ITEMS'][$_POST['dataRemove']]);
}
if($_POST['dataRemoveFlag']){
	$_SESSION[$arParams['NAME']][$arParams['IBLOCK_ID']]['kol_all']--;
}

//foreach($_SESSION['CATALOG_CART_LIST'][3]['ITEMS'] as &$item){
//	if($item['kol']==0){
//		$_SESSION['CATALOG_CART_LIST'][3]['ITEMS'][$item['ID']] = [];
//	}
//}
