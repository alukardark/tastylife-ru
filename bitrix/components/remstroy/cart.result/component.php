...
if (isset($_GET['delete_cart_all'])) unset($_SESSION[$arParams["NAME"]]);
...
if (!empty($arCompare) && is_array($arCompare))
{
	...
	while($obElement = $rsElements->GetNextElement())
	{
		...
		// $arItem["kol"] = $arItem["ID"];
		$arItem["kol"] =$_SESSION['CATALOG_CART_LIST'][$arParams["IBLOCK_ID"]]['ITEMS'][$arItem["ID"]]['kol'];
		$arItem["pict"] =$_SESSION['CATALOG_CART_LIST'][$arParams["IBLOCK_ID"]]['ITEMS'][$arItem["ID"]]['pict'];
		// $arItem["kol"] =$_SESSION['CATALOG_CART_LIST'][$arParams["IBLOCK_ID"]]['ITEMS'][$arItem["ID"]]['kol'];
...
?>
	...
	<div class="row">
		...
		<i class="fa fa-trash-o" style="font-size: 120px; color: #c27272"></i>	
		</div>
	...
	<?=GetMessage("CATALOG_COMPARE_LIST_EMPTY");?>
			
			</h3>
					<br>
					<a href="<?=SITE_DIR?>" class="btn btn-default order-finish_home"> <?=GetMessage('home')?> </a> 
					<!-- <a href="<?=SITE_DIR?>catalog/" class="btn"> <?=GetMessage('catalog')?> </a>  -->
				</div>
			</div>	
</div>
<?
